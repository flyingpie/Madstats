<?
	...
	class FTP {
		...
		function fput($remote,$resource,$mode=FTP_BINARY,$startpos=0) { //Uploads from an open file to the FTP server
			...
			if($mode == FTP_ASCII) $result = $this->_uploadAsciiResource("STOR $remote", $resource);
			else $result = $this->_uploadResource("STOR $remote",$resource);
			return $result;
		}
...
*/
		function get($remote) {
			$result = $this->fget(null, $remote, FTP_ASCII, 0);
			return $result;
		}
		...
		function putAscii($remote, $contents) {
			$result = $this->fput($remote, $contents, FTP_ASCII, 0);
			return $result;
		}
		...
		function _uploadAsciiResource($cmd, $resource) {
			if (!($passiveConnection = $this->_pasv())) return FALSE;
			$this->_printCommand($cmd);
			$this->_waitForResult();
			$lr = $this->getLastResult();
			if (!$this->_isError()) {
				$result = "";
				fwrite($passiveConnection, $resource);
				fclose($passiveConnection);
				$this->_waitForResult();
				$lr = $this->getLastResult();
				return ($lr==FTP_FILE_TRANSFER_OK) || ($lr==FTP_FILE_ACTION_OK) || ($lr==FTP_COMMAND_OK) ? $result : FALSE;
